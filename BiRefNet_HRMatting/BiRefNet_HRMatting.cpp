/*
 * BiRefNet_HRMatting - TensorRT BiRefNet HR Matting Node for Nuke
 *
 * Model details (confirmed via Netron / trtexec):
 *   Input:  "input_image"  float32[1,3,2048,2048]  (NCHW)
 *   Output: "output_image" float32[1,1,2048,2048]  (NCHW)
...
public:
    void log(Severity severity, const char* msg) noexcept override
    {
        if (severity <= Severity::kWARNING)
            std::cerr << "[BiRefNet_HRMatting] " << msg << std::endl;
    }
};
...
// ---------------------------------------------------------------------------
static const int kModelW = 2048;
static const int kModelH = 2048;
...
// ---------------------------------------------------------------------------
// BiRefNet_HRMatting node
// ---------------------------------------------------------------------------
class BiRefNet_HRMatting : public Iop
{

public:

    BiRefNet_HRMatting(Node* node);
    ~BiRefNet_HRMatting() override;

    const char* Class() const override { return description.name; }
    const char* node_help() const override
    {
        return "BiRefNet HR Matting — high-resolution portrait matting.\n\n"
               ...
               "Model resolution: 2048x2048\n"
               "Engine is embedded — no external files needed.\n\n"
               "BiRefNet_HRMatting for Nuke by Peter Mercell, 2026\n"
...
// Ctor / Dtor
// ---------------------------------------------------------------------------
BiRefNet_HRMatting::BiRefNet_HRMatting(Node* node)
    : Iop(node)
...
BiRefNet_HRMatting::~BiRefNet_HRMatting()
{
    freeGPU();
    freeEngine();
}
...
// ---------------------------------------------------------------------------
// Knobs — minimal UI: output controls + credits
// ---------------------------------------------------------------------------
void BiRefNet_HRMatting::knobs(Knob_Callback f)
{
    ...
    // Hidden but active — ImageNet normalization always on
    Bool_knob(f, &useImageNetNorm_, "imagenet_norm", "ImageNet Normalize");
    SetFlags(f, Knob::HIDDEN);

    // Hidden but active — GPU device selection
    Int_knob(f, &gpuDevice_, "gpu", "GPU Device");
    SetFlags(f, Knob::HIDDEN);

    Divider(f, "");
    Text_knob(f, "BiRefNet_HRMatting for Nuke by Peter Mercell, 2026\n"
                 "\n"
                 "BiRefNet model by Peng Zheng\n"
                 "www.github.com/ZhengPeng7/BiRefNet (MIT License)\n"
...
// ---------------------------------------------------------------------------
void BiRefNet_HRMatting::loadEngine()
{
    ...
    std::cerr << "[BiRefNet_HRMatting] Loading embedded engine ("
    ...
    engineLoaded_ = true;
    std::cerr << "[BiRefNet_HRMatting] Embedded engine loaded successfully."
              << std::endl;
}

void BiRefNet_HRMatting::freeEngine()
{
...
// GPU buffers
// ---------------------------------------------------------------------------
void BiRefNet_HRMatting::allocateGPU()
{
    freeGPU();
...
void BiRefNet_HRMatting::freeGPU()
{
...
void BiRefNet_HRMatting::_close()
{
    freeGPU();
    freeEngine();
}
...
// _validate
// ---------------------------------------------------------------------------
void BiRefNet_HRMatting::_validate(bool for_real)
{
    copy_info();
...
// ---------------------------------------------------------------------------
void BiRefNet_HRMatting::_request(int x, int y, int r, int t,
...
// ---------------------------------------------------------------------------
void BiRefNet_HRMatting::fetchAllRows()
{
...
// ---------------------------------------------------------------------------
void BiRefNet_HRMatting::preprocessFrame()
{
...
// ---------------------------------------------------------------------------
void BiRefNet_HRMatting::postprocessMatte()
{
    int n = kModelW * kModelH;
...
// ---------------------------------------------------------------------------
void BiRefNet_HRMatting::runInference()
{
    ...
    if (!ok)
        std::cerr << "[BiRefNet_HRMatting] enqueueV3 FAILED!" << std::endl;
...
// ---------------------------------------------------------------------------
void BiRefNet_HRMatting::doFullInference()
{
    fetchAllRows();
...
// ---------------------------------------------------------------------------
void BiRefNet_HRMatting::engine(int y, int x, int r,
...
{
    return new BiRefNet_HRMatting(node);
}

const Iop::Description BiRefNet_HRMatting::description(
    "BiRefNet_HRMatting",
    "AI/BiRefNet_HRMatting",
    build
);
